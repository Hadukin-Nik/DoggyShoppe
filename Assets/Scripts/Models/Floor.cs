...
public class Floor : MonoBehaviour
{
    ...
    private KeyValuePair<int, int> _realMatrixSize;
    void Start()
    {
        _correctForwardVector = _pointStart.forward * _matrixDivisionUnit;
        _correctRightVector = _pointStart.right * _matrixDivisionUnit;

        _realMatrixSize = new KeyValuePair<int, int>((int)(_size.x / _matrixDivisionUnit), (int)(_size.z / _matrixDivisionUnit));
    ...
    {
        List<KeyValuePair<BuildingContoller, List<KeyValuePair<int, int>>>> buf = _buildingsHandler.GetAll();
        if (buf.Count > 0)
            return _pathFindingController.GetWay(_buildingMatrixController.getBuildingMatrix(), _buildingMatrixController.fromGlobalToMatrix(startPoint), buf[buf.Count - 1].Value);
    ...
    public List<Vector3> GetWayToRandom(Vector3 startPoint)
    {
        List<KeyValuePair<BuildingContoller, List<KeyValuePair<int, int>>>> buf = _buildingsHandler.GetAll();
        if (buf.Count > 0)
            return _pathFindingController.GetWay(_buildingMatrixController.getBuildingMatrix(), _buildingMatrixController.fromGlobalToMatrix(startPoint), buf[Random.Range(0, buf.Count)].Value);
